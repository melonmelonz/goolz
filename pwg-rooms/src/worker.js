// Playing With Goolz multiplayer relay — separate Worker so the Pages
// project can bind the DO via `script_name`. Cloudflare Pages does not
// allow inline DO declarations.
//
// Hardening:
//  - max 24 concurrent connections per room
//  - max 64 KB per inbound message
//  - per-socket token-bucket rate limit (~25 msg/s, burst 40)
//  - text fields sanitized; reserved control types never relayed
//  - sender id server-stamped; allow-listed fields only
...
// Default fetch handler — the goolz Pages Function calls
// env.ROOMS.idFromName(code).fetch(req) directly, but exporting a fetch
// handler also lets this Worker be reachable on its own URL for testing.
export default {
  async fetch(req, env) {
    const url = new URL(req.url);
    const code = (url.pathname.split('/').filter(Boolean).pop() || 'LOBBY')
      .toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 8) || 'LOBBY';
